...
- 账号凭证加密保存。
- 余额刷新，签到后显示余额变化。
- 手动或按账号/站点时区定时签到，并保留账号级结果历史。
- 同步账号模型，使用站点账号直接测试模型。
- 聚合站点公告并记录已读状态。
- 低余额、签到失败等事件的通用 Webhook 与 Telegram 通知。
- 将站点账号幂等投影为路由渠道。
...
- 主题、路由、冷却、日志、自动签到时间、通知和只读上游版本检查。
- 支持完整配置、连接与路由、系统设置三类 JSON 导入导出，并可定时备份到 WebDAV。
...
1. 在“系统设置”检查自动签到时间、通知、备份、请求上限、冷却和日志。
