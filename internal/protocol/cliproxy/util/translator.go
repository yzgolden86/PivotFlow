...
// CanonicalToolName returns the stable protocol-safe representation of a tool name.
...
// MapToolName restores a mapped tool name when one is registered.
...
func SanitizedToolNameMap(rawJSON []byte) map[string]string {
	...
	tools.ForEach(func(_, tool gjson.Result) bool {
		...
		if existing, exists := out[sanitized]; !exists {
			...
			log.Printf("sanitized tool name collision: %q and %q both map to %q, keeping first", existing, name, sanitized)
		}
		return true
	})
	if len(out) == 0 {
