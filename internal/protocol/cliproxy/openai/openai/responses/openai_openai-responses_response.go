...
func ConvertOpenAIChatCompletionsResponseToOpenAIResponses(ctx context.Context, modelName string, originalRequestRawJSON, requestRawJSON, rawJSON []byte, param *any) [][]byte {
	...
	if bytes.Equal(rawJSON, []byte("[DONE]")) {
		var out [][]byte
		if st.Started && !st.CompletionPending {
			terminal := []byte(`{"id":"","object":"chat.completion.chunk","created":0,"choices":[{"index":0,"delta":{},"finish_reason":"stop"}]}`)
			terminal, _ = sjson.SetBytes(terminal, "id", st.ResponseID)
			terminal, _ = sjson.SetBytes(terminal, "created", st.Created)
			out = append(out, ConvertOpenAIChatCompletionsResponseToOpenAIResponses(context.Background(), "", originalRequestRawJSON, requestRawJSON, terminal, param)...)
		}
