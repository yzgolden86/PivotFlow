---
name: pivotflow-release
description: 用于发布 PivotFlow 新版本，自动提交未提交改动并推送本地领先的 main，按固定版本通道计算并发布 Tag，等待 GitHub Actions，以及验证 GitHub Release 和对应通道的容器镜像。
---
...
- `$pivotflow-release`、`/pivotflow-release`、`pivotflow-release beta`、`pivotflow-release preview`：发布下一个 Beta。
- `pivotflow-release stable`：发布稳定版。必须有显式 `stable` 参数；禁止根据语气猜测。
...
2. 从仓库根目录运行预览。默认渠道是 `beta`。工作区干净时运行：
   ...
   ```bash
   bash .agents/skills/pivotflow-release/scripts/release.sh beta --dry-run
   ```

   工作区有改动时传入推导出的提交 subject：

   ```bash
   bash .agents/skills/pivotflow-release/scripts/release.sh beta --dry-run --commit-message 'feat(scope): summary'
   ```
...
4. 执行发布。工作区干净时运行：
   ...
   ```bash
   bash .agents/skills/pivotflow-release/scripts/release.sh beta --publish
   ```

   工作区有改动时必须复用 dry-run 的同一个 subject：

   ```bash
   bash .agents/skills/pivotflow-release/scripts/release.sh beta --publish --commit-message 'feat(scope): summary'
   ```

   稳定版把 `beta` 改为 `stable`。脚本会自动 `git add -A`、创建提交、运行全部发布门禁、非强制推送 `master`，确认远端精确一致后再创建并推送 annotated Tag。本地已有未推送提交时不创建额外提交，验证通过后直接推送。

5. 报告自动创建的提交（如有）、推送的 `main` 修订、目标 Tag、GitHub Release URL 和 Actions 结果。稳定版报告 `ghcr.io/yzgolden86/pivotflow:<tag>` 和 `ghcr.io/yzgolden86/pivotflow:latest`；Beta 报告 `ghcr.io/yzgolden86/pivotflow:<tag>` 和 `ghcr.io/yzgolden86/pivotflow:beta`。

## 强制规则

- 当前分支必须是 `main`。本地 `main` 可与 `origin/main` 相等或领先；远端领先或双方分叉时停止，禁止自动 pull、merge、rebase 或 force-push。
...
- 分支推送只能是普通 fast-forward push。推送后必须重新 fetch 并确认本地 `HEAD` 等于 `origin/main`，才能创建 Tag。
...
```bash
bash .agents/skills/pivotflow-release/scripts/release.sh --self-test
```
