...
import { CalendarCheck2, ExternalLink, History, Info, Play, RefreshCw, Users } from 'lucide-react'
...
export default function CheckinsPage() {
  ...
  // 签到失败原因要能直接指引下一步动作：不支持签到和需要浏览器验证是两种完全不同的
  // 处理方式，前者只能去上游手动签，后者点开签到页完成验证就能回来重试。
  const checkinFailure = (reason: unknown, account: SiteAccount) => {
    const raw = reason instanceof Error ? reason.message : String(reason || '')
    const separator = raw.indexOf(': ')
    const code = separator > 0 ? raw.slice(0, separator) : raw
    const browserURL = siteBrowserCheckinURL(siteMap.get(account.site_id))
    if (code === 'unsupported') return browserURL ? '该站点不支持自动签到，可点左侧“打开签到页”手动完成' : '该站点不支持自动签到'
    if (code === 'browser_required') return browserURL ? '该站点需要浏览器验证：点左侧“打开签到页”完成上游操作后回来重试，系统会识别今日已签到' : siteErrorMessage(reason)
    if (code === 'already_checked') return '该账号今日已签到'
    return siteErrorMessage(reason)
  }
  const runOne = async (account: SiteAccount, quiet = false) => { setBusy((current) => new Set(current).add(account.id)); setError(''); try { const result = await executeAccountTask(account.id, 'checkin'); if (!['success', 'partial'].includes(result.status)) throw new Error(result.error || result.status); if (!quiet) { setNotice(`${account.label} 签到完成`); await load(undefined, { silent: true, force: true }) } return true } catch (reason) { if (!quiet) setError(checkinFailure(reason, account)); return false } finally { setBusy((current) => { const next = new Set(current); next.delete(account.id); return next }) } }
...
function AccountCheckinList({ accounts, siteMap, busy, run }: { accounts: SiteAccount[]; siteMap: Map<number, Site>; busy: Set<number>; run: (account: SiteAccount) => void }) {
  ...
  return <div className="records-panel checkin-account-records"><div className="record-head checkin-account-grid"><span>账号 / 站点</span><span>账号状态</span><span>最近签到</span><span>自动签到</span><span>操作</span></div>{accounts.map((account) => { const site = siteMap.get(account.site_id); const apiKeyOnly = account.credential_type === 'api_key'; const disabled = !account.enabled; const needsCredential = ['expired', 'error'].includes(account.status) || Boolean(account.last_error); const platformCheckin = platformSupportsCheckin(site?.platform || ''); const checkinSupported = !apiKeyOnly && platformCheckin; const browserCheckinURL = siteBrowserCheckinURL(site); return <article className="record-row checkin-account-grid" key={account.id}><div><a className="entity-link" href={`#/accounts?focus_account_id=${account.id}`}><strong>{account.label}</strong></a><a className="entity-chip" href={`#/sites?focus_site_id=${account.site_id}`}>{site?.name || `站点 #${account.site_id}`}</a></div><div><StatusBadge status={account.enabled ? account.status : 'disabled'} />{needsCredential ? <a className="inline-entity-action" href={`#/accounts?focus_account_id=${account.id}&open_credential=1`} title={account.last_error}>{account.last_error ? siteErrorMessage(account.last_error) : '更新凭证'}</a> : <span className="account-error-message">凭证已配置</span>}</div><div className="checkin-recent"><StatusBadge status={apiKeyOnly ? 'unsupported' : account.last_checkin_status} /><span>{apiKeyOnly ? '模型 API Key 不支持签到' : account.last_checkin_at ? formatTime(account.last_checkin_at) : '尚未签到'}</span></div><div><strong>{apiKeyOnly ? '不支持' : account.auto_checkin ? '已开启' : '未开启'}</strong><span>{account.timezone || site?.timezone || '站点时区'}</span></div><div className="checkin-actions">{checkinSupported ? <>{browserCheckinURL && <a className="secondary-button" href={browserCheckinURL} target="_blank" rel="noreferrer" title="在上游站点手动完成签到或验证">打开签到页<ExternalLink size={12} /></a>}<button className="primary-button" type="button" onClick={() => run(account)} disabled={disabled || busy.has(account.id)}>{busy.has(account.id) ? <RefreshCw className="spin" size={13} /> : <Play size={13} />}{disabled ? '账号已停用' : '立即签到'}</button></> : <span className="checkin-unsupported" title={apiKeyOnly ? '模型 API Key 只用于调用模型与路由，不携带签到凭证' : `平台 ${site?.platform || 'unknown'} 没有服务端签到接口，可在站点管理填写浏览器辅助签到地址`}><Info size={13} />{apiKeyOnly ? '模型 API Key 不支持签到' : '该平台不支持签到'}</span>}</div></article> })}</div>
}
