...
export default function AccountsPage() {
  ...
  const openCredential = useCallback((account: SiteAccount) => {
    ...
    // A refreshed access token gets a new expiry from the provider. Keep this
    // field empty when editing so an old timestamp cannot be carried forward.
    next.expires_at = ''
  ...
  return <div className="workspace-page">
    ...
    {routeSyncRun && <Modal title="批量同步路由" close={() => { if (!routeSyncRun.running) setRouteSyncRun(null) }} wide><RouteSyncProgress run={routeSyncRun} close={() => setRouteSyncRun(null)} /></Modal>}
...
function CredentialUpdateView({ account, site, form, change, verification, verifying, saving, verify, save }: {
  ...
  return <div className="credential-update">
    <div className="credential-alert"><KeyRound size={17} /><div><strong>{account.label}</strong><span>{site?.name || `站点 #${account.site_id}`} · 当前为 {credentialLabel(account.credential_type, site?.platform)}</span>{account.credential_configured && <small>已保存的凭证经加密存储、无法读回，下方留空表示保持不变；填入内容即整体替换。</small>}{account.last_error && <small>{siteErrorMessage(account.last_error)}</small>}</div></div>
