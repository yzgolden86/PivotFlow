...
export default function AccountsPage() {
  ...
  const remove = async (account: SiteAccount) => { if (!window.confirm(`删除账号“${account.label}”？对应投影渠道将被禁用。`)) return; setBusy((current) => new Set(current).add(account.id)); try { await deleteSiteAccount(account.id); setNotice('账号已删除'); await load() } catch (reason) { setError(siteErrorMessage(reason)) } finally { setBusy((current) => { const next = new Set(current); next.delete(account.id); return next }) } }
  ...
  return <div className="workspace-page">
    ...
    {notice && <OperationNotice onDismiss={() => setNotice('')}>{notice}</OperationNotice>}{error && accounts.length > 0 && <div className="inline-error">{error}</div>}
