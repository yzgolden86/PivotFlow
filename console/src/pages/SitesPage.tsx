  ...
  return <form className="console-form" onSubmit={submit}>
	...
	{!editing && <section className="embedded-form-section">
      ...
      {form.addAccount && <>
        ...
        {form.platform === 'sub2api' && form.credentialType === 'access_token' && <><div className="form-grid"><label><span>Refresh Token（可选）</span><SecretInput value={form.refreshToken} change={(value) => field('refreshToken', value)} placeholder="用于访问令牌自动续期" /></label><label><span>访问令牌过期时间（可选）</span><input type="text" inputMode="numeric" pattern="[0-9]*" value={form.expiresAt} onChange={(event) => field('expiresAt', event.target.value.replace(/[^0-9]/g, ''))} placeholder="例如 1786982260314" /></label></div><div className="form-help">填写 Auth Token；如同时提供 Refresh Token，系统会在访问令牌即将过期前自动续期。</div></>}
